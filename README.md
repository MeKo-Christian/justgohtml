...
Go's performance means JustGoHTML is significantly faster than pure-Python parsers while maintaining 100% spec compliance. After Phase 3.2 optimizations (dispatch table + ring buffer), it parses complex HTML pages at **~8,300 pages per second** (121 µs per 5KB page). CSS selector matching is now **faster than goquery** for many queries.
...
| `golang.org/x/net/html` |      96.2%       |   Yes    | **1.9x**    | —           | None          | Good but fails 60 of 1,578 tree tests.    |
| `goquery`               |      96.2%       |   Yes    | **1.7x**    | 0.64x       | CSS selectors | Wrapper around x/net/html.                |
| `htmlquery`             |      96.2%       |   Yes    | ~1.9x       | —           | XPath         | Uses x/net/html parser + XPath queries.   |

**Compliance**: Measured against html5lib-tests tree-construction suite (1,578 non-scripted, non-fragment tests).

- JustGoHTML: 1,578/1,578 passed (100.0%)
- net/html: 1,518/1,578 passed (96.2% — 60 failures including foster parenting, adoption agency, foreign content)

**Parse Speed**: Complex HTML (~5KB). net/html is ~1.9x faster but less compliant. JustGoHTML uses 36-44% less memory than before Phase 3.2.4.3.

**Query Speed**: JustGoHTML CSS selectors are now **faster than goquery** for equivalent queries (~35-40% faster).
